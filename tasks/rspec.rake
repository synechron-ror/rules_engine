...
task :spec => ['spec:setup', 'spec:rules_engine_lib', 'spec:model', 'spec:routing', 'spec:controller', 'spec:cleanup']
...
namespace :spec do
  ...
  task :cleanup do
    ...
    FileUtils.rm_rf("#{rails_path}/public/stylesheets/rules_engine")
    FileUtils.rm_rf("#{rails_path}/public/javascripts/rules_engine")
    FileUtils.rm_rf("#{rails_path}/config/initializers/rules_engine.rb")
    FileUtils.rm_rf("#{rails_path}/db/migrate/20100308225008_create_rules_engine.rb")
    FileUtils.rm_rf("#{rails_path}/doc/README.rules_engine")
